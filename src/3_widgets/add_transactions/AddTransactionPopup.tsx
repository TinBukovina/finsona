...
export function AddTransactionsPopup({
  flowing = true,
  onClose,
  ...
  return (
    ...
    >
      {/*EXIT AND CLOSE BTN*/}
      <div className="flex gap-2 fill-card-foreground">
        <div
          className="w-fit h-fit hover:fill-primary cursor-pointer"
          onClick={onClose}
        >
          <IconTemplate svg={close_r_400()} width="24px" height="24px" />
        </div>
        <div
          className={`w-fit h-fit ${hideContent ? "-rotate-90" : "rotate-90"} hover:fill-primary cursor-pointer`}
          onClick={() => setHideContent((prev) => !prev)}
        >
          <IconTemplate svg={last_page_r_400()} width="28px" height="24px" />
        </div>
      </div>
